...
*Q:* Can we identify examples of glTF usage in the Gazebo community that would inform, bottom-up, what folks are using as best practices?

*A (CC):* Yes, and the answer outgrew this note. The bottom-up survey now lives in `maritime-workspace/gltf-assess/`, where it is re-run rather than rewritten: `criteria.md` states what is assessed and why, `examples.yaml` is the list of assets, `gltf_assess.py` measures them over HTTP range requests without downloading them whole, and `ASSESSMENT.md` and `findings.md` are what comes out. The Fuel survey of 2026-09-12 that used to sit here is the opening section of `findings.md`.

The short version, so this note is not left with a hole in it: glTF is nearly absent from Fuel -- 54 files among 2,333 models, three of them robots -- so there is no community practice for glTF robots to copy, and this project's specification is doing original work rather than following anyone. The Gazebo team's own demo assets are the nearest thing to a reference, and they disagree with several rules in `model-spec.md`.
